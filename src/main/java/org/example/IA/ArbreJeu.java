...
public class ArbreJeu{
    ...
    public static void construireArbre2(Node racine,int profondeur ){
        ...
        for(int i=0 ; i < racine.plateau.getJoueurCourant().getHandScndPhase().size();i++){
            Plateau copie = new Plateau(racine.plateau);
            Card cardi = copie.getJoueurCourant().getHandScndPhase().get(i);
            ...
            for(Card cardj : carteJouable){
                ...
                Card carteGagnant=carteGagnante(cardi,cardj , copie);

                System.out.println(copie.getJoueur1().getName());
                System.err.println(copie.getCarteJoueur1()+"");
                System.out.println(copie.getJoueur2().getName());
                System.err.println(copie.getCarteJoueur2());
                System.err.println("****** Carte Gagnante  " + carteGagnant + " ******");
                System.err.println("-----------------------------------------------------");
                Player x=copie.getJoueurCourant();
                copie.attribuerCarteSecondPhase(carteGagnant,new ReglesDeJeu());

                Node enfant = new Node(new Plateau(copie));
    ...
    public static List<Card> cartesJouables2(Card carteAd, Plateau racine) {
        List<Card> carteJoueable= ReglesDeJeu.cartesJouables(carteAd, racine.getJoueurCourant().getHandScndPhase());
        Player joeurCourant=racine.getJoueurCourant();
        carteJoueable.removeAll(joeurCourant.carteDejaJouee);
        return carteJoueable;
    }
